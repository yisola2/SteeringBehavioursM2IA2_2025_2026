...
class SliderPanel {
  ...
  constructor(x = 20, y = 20, {
    ...
    // --- titre optionnel (coloré selon le comportement) ---
    if (title) {
      this._titleHeight = gap;
      ...
      titleDiv.style('text-transform', 'uppercase');
      titleDiv.style('letter-spacing', '1px');
    }

    this._y = y + this._titleHeight; // les sliders commencent après le titre
  }

  /** Hauteur totale occupée par ce panneau (titre + sliders). */
  height() {
    return this._titleHeight + this._count * this._gap;
  }
